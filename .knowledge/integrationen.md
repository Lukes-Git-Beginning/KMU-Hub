...
- **Offen (minor):** G4 RevokeTokens-Vault-Orphan, G6 (veraltet, s.u.), G7 org_name nie befüllt, G8 (für Pull-Pfad gefixt; Contact/Payment latent mit `lean:`-Marker, s.u.), G9 (veraltet, s.u.)
- **G12 UMGESETZT (2026-07-05):** Invoice-Pull = **Read-only Spiegel** (NICHT voll bidirektional), Delta-forward (keine Historie) — Phase 2–4 ✅ gebaut+deployt (Details unten unter „Welle 3b"). Darien-GoBD-Gegenzeichnung noch offen.
...
- **G6/G9 aus der Härtungs-Liste sind VERALTET:** `PUT /sync/config` existiert (`route_bexio.go`, G6 falsch); `integrations.bexio`-Flag existiert (`featureflag/registry.go:97`, G9 falsch). **G8 für den Pull-Pfad GEFIXT (Phase 2):** der neue `invoicePullTicker` reicht `ts.configID` explizit durch (`Service.PullInvoicesWithConfig`) statt `getConfigID(ctx)` unter System-Context (RLS-Bypass) neu aufzulösen. Contact-/Payment-Ticker behalten denselben latenten Bug mit `lean:`-Marker (heute Single-Tenant harmlos; vor Multi-Tenant-Aktivierung ebenfalls `ts.configID` durchreichen).
...
**Phase 2 ✅ (`64366f27`, CD-deployt):** `invoice_pull.go`-Puller (`InvoicePuller`, Vorbild `ContactSyncer.syncInbound`) + schmales `InvoiceImporter`-Interface (`UpsertImported`+`GetByID`, erfüllt von `*invoice.Service`), verdrahtet in `cmd/biz/main.go`. Loop-Prevention (Skip wenn Ziel-`source='cosmi'` via `GetByID`), LWW auf `bexio_updated_at`, neue Mappings `sync_direction='inbound'`. **Delta-forward-Guard** im Puller: nil-Cursor → `last_invoice_pull_at=now` seeden + return (keine Historie; ein Enforcement-Punkt für Ticker + Manual-Trigger). Scheduler-3.-Ticker `invoicePullTicker` (G8-korrekt, s.o.). 5 Puller-Unit-Tests (httptest-Client). `Service.PullInvoices` (gRPC-Pfad) + `PullInvoicesWithConfig` (Scheduler-Pfad).
**Phase 3a ✅ (`7767c7cd`, CD-deployt):** Proto `invoice_pull_enabled`/`_interval_minutes`/`last_invoice_pull_at` in Status+Config; `bexio.proto` regeneriert (voller `protoc`-Befehl — `make proto-biz` deckt es jetzt mit ab, Bestandslücke geschlossen; Regen brachte `bexio.pb.go` auf `file_proto_*`-Prefix konsistent zu `biz.pb.go`). gRPC `case "invoices"` → `PullInvoices`; `syncStatusToProto`/`UpdateBexioSyncConfig` + Felder. Gateway (flach) `HandleGetSyncStatus`/`HandleUpdateSyncConfig` + Felder. OpenAPI-Schemas ergänzt.
**Phase 3b ✅ (`e19a68fb`, CI Desktop grün):** FE via Worktree-Subagent. `bexio-types.ts` toter verschachtelter `BexioSyncStatus` → **flache Wire-Shape** (Gateway ist flach); `bexio-client.ts` Pfad-Drift gefixt (`/auth-url`→`/oauth/authorize`, `/sync`→`/sync/trigger`) + neue `updateSyncConfig` + `listSyncLogs` bare-array; `useBexioUpdateSyncConfig`-Hook. Wizard: `invoice_pull`-Toggle + **`handleFinish` persistiert Config** (war nie gespeichert). Dashboard-Karten auf flache Felder, „Invoices"-Karte = Invoice-Pull-Status. Invoice-Detail: `source='bexio'` → Read-only-Badge + Banner, **alle mutierenden Aktionen ausgeblendet** (Edit/Send/RecordPayment/MarkPaid/Cancel/Storno — Backend guardet `MarkPaid`; Bexio besitzt Zahlstatus). i18n ×4. Screenshot-QA: Invoice-Detail read-only visuell verifiziert.
**Phase 4 ✅:** Backend-Tests grün (Integration PG16 + Unit); Prod `GET /integrations/bexio/status`→401 (Route live). **Echter Bexio-Pull = manueller Sandbox-Test offen** (kein Test-Account; Mapper/Puller-Logik via Unit-Tests + Delta/LWW/Skip abgesichert). Darien-GoBD-Gegenzeichnung von G12 weiterhin offen.
