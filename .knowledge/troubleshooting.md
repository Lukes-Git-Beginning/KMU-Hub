...
- **plugin NICHT migrierbar:** `backend/proto/plugin/v1/plugin.pb.go` ist gefälscht (`DO NOT EDIT`-Header, aber `grep -c ProtoReflect` = 0 → Typen erfüllen `proto.Message` nicht → `response.Proto` bricht den Build). Timestamp-Bug (ExecutionLog/PluginDetail „Invalid Date") bleibt bis echte `protoc`-Regenerierung. plugin ist das **einzige** Fake-pb.go — Systemcheck: `grep -rc ProtoReflect backend/proto/*/v1/*.pb.go` (nur plugin = 0).
- **map-envelope-Falle:** Handler wie `map[string]any{"folder": resp.Folder}` reichen den verschachtelten Proto durch — `encoding/json` serialisiert dessen Timestamps weiterhin als `{seconds,nanos}`. Fix: verschachtelten Wert lokal protojson→`json.RawMessage` marshalen (so bei inbox canned-response) ODER `response.Proto(resp)` wenn die Response-Message dieselbe Envelope-Form hat. document/hr map-wrapper bewusst offen (`lean:`-Marker in `route_document.go`; FE normalisiert client-seitig, `file_size` muss numerisch bleiben).
- **Offen (nach Launch):** Welle 3–6 (~38 Admin-/Integrations-Routen: biz/bexio/lexware/datev/auth/security/settings/chat/email/automation/formulare/berichte). Referenz-Pattern: `route_dialer.go`/`route_video.go`/`route_rapporte.go`.
