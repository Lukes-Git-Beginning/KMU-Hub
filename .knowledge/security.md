...
- **Inbound** (`middleware.TenantInboundUnaryInterceptor`) — **R3-P0-3 erledigt seit `777b27ad` (2026-06-20):** in allen 23 gRPC-cmd-Binaries verdrahtet (die urspruenglichen 10 — auth, biz, chat, work, document, crm, dialer, email, formulare, notification — plus die 13 nachgezogenen wiki, helpdesk, berichte, inventar, einkauf, produktion, schichten, vermietung, fuhrpark, rapporte, vertraege, automation, plugin). Verifiziert 2026-08-06 per `grep -l TenantInboundUnaryInterceptor cmd/*/main.go` gegen `ls -d cmd/*/` (23/23, `cmd/gateway` ausgenommen — der ist Aufrufer, kein gRPC-Server).
