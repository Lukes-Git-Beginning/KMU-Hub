...
Das ist kein RLS-Luecken-Fall im Sinne dieses Abschnitts (RLS wuerde ohne `tenant_id` nichts trennen) und keine reine Katalog-/Seed-Tabelle (der Inhalt IST admin-mutable und divergiert potenziell pro Tenant) — beide Antworten dieses Abschnitts passen nicht. Die ehrliche Loesung braucht `tenant_id` + Backfill + ein Provisioning-Schritt fuer neue Tenants (`GetConfig`/`GetDefaults` holen genau eine globale Zeile per `LIMIT 1` — ohne eigene Zeile pro Tenant faellt der Read nach RLS auf `ErrNoRows`). Der Provisioning-Ort steht fest: `PostgresRepository.ProvisionTenant` (`internal/auth/postgres_repository.go`) legt Tenant, Modul-Aktivierungen und Erst-Einladung in einer Transaktion an. Backlog-Units `g-rls-presence-and-dashboard-defaults`, `g-rls-storage-quotas`, `g-rls-plugin-manifests` (letztere mit offener Produktfrage: Tenant-Admin- oder Plattform-Operation).
