...
**Offener Befund — tenant-uebergreifende Schreibpfade unter einem tenant-gescopten Guard:** zwei weitere Tabellen ohne `tenant_id` werden ueber eine mit `RequireRole("admin")` geschuetzte Route bzw. ohne WHERE-Klausel geschrieben — der Guard prueft nur "ist Admin/hat Recht", nicht "in welchem Tenant":
