...
**Geschlossen — `user_roles` (seit Migration 000286):** letzte Tabelle des RBAC-Kerns ohne eigene Absicherung. `tenant_id NOT NULL` per Backfill aus `users.tenant_id` (FK garantiert keine Waisen) + `enable_tenant_rls()`. Bewusst **keine** Split-Policy wie bei `roles`/`role_permissions`: eine `user_roles`-Zeile ist immer eine ZUWEISUNG und gehoert dem Tenant des Accounts — die Rolle dahinter mag ein Preset sein, die Zuweisung ist es nie.

Der schaerfere Teil des Fundes liess sich damit aber nicht schliessen und sitzt in Go, nicht in der Policy: `AssignRole`/`RemoveRole` loesen die Rolle ueber ihren Namen auf und laufen bei der Registrierung unter `sysctx.With()` (`Register` haengt jeden neuen Account an "member") — im System-Kontext filtert RLS nichts. Seit Migration 000256 darf ein Tenant eine eigene Rolle genauso nennen wie ein Preset (`idx_roles_tenant_name` kollidiert nicht, weil Presets `tenant_id IS NULL` fuehren), also matchte `WHERE r.name = $2` **beide** Zeilen und haette jeden neuen Registranten zusaetzlich in die gleichnamige Custom-Rolle eines fremden Tenants gehaengt — eine Rechte-Uebertragung ueber die Tenant-Grenze, ausgeloest durch nichts weiter als eine Registrierung. Beide Funktionen loesen jetzt ausschliesslich System-Presets auf (`r.tenant_id IS NULL`); Custom-Rollen laufen nur noch ueber den ID-basierten `AssignUserRole`/`RevokeUserRole`-Pfad. Regressionstest: `TestAssignRole_DB_PresetOnlyDespiteNameCollision`.

Merksatz fuer die naechste Tabelle dieser Art: **RLS ist unter `sysctx.With()` wirkungslos** — was dort passieren darf, muss die Query selbst einschraenken.
...
**Geschlossen — `plugin_manifests` (seit Migration 000276):** fuenfter und letzter der Faelle. `POST /api/v1/plugins/manifests` war nur mit `RequireRole("admin")` geschuetzt und schrieb ohne `tenant_id`, `GET /manifests` las ohne Filter — jedes `plugin_type=config`-Manifest eines Tenants war fuer alle sichtbar und installierbar (WASM-Manifeste blockt der `plugins.wasm`-Flag-Check gesondert, Config-Manifeste nicht). Dieselbe Luecke machte `DELETE /manifests/{id}` tenant-uebergreifend destruktiv: `DeleteManifest` verweigert das Loeschen bei aktiven Installationen, zaehlt sie aber ueber `plugin_installations`, das seit 000122 RLS hat — die Installationen fremder Tenants waren fuer die Zaehlung unsichtbar, der Count kam als 0 zurueck, und `manifest_id ON DELETE CASCADE` riss sie mit.

Die Frage, die diesen Fall von den vier anderen unterschied — Tenant- oder Plattform-Operation — ist mit einem **nullable `tenant_id`** beantwortet, dem Policy-Paar, das `roles`/`role_permissions` seit 000256 tragen: `NULL` heisst "mit dem Produkt ausgeliefert, fuer jeden Tenant lesbar, fuer keinen schreibbar", ein gesetzter Tenant heisst "eigenes Manifest". Lesen sieht beides, Schreiben nur das eigene. Das deckt den Katalog UND die Tenant-Manifeste in einem Schema ab, ohne die Plattform-Rolle, die es heute nicht gibt, und ohne Tenants die Faehigkeit zu nehmen, eigene Config-Plugins zu definieren. Katalog-Zeilen entstehen wie die Rollen-Presets per Migration, nicht ueber eine Route. Kein Backfill: bestehende Zeilen bleiben `NULL` — sie waren fuer alle sichtbar und bleiben es.

Zwei Punkte aus der Umsetzung:
- **Ein Schreibversuch, den nur die Policy abfaengt, meldet Erfolg.** Ein `DELETE` auf eine Katalog-Zeile trifft unter der Write-Policy schlicht null Zeilen und liefert 200. `Service.DeleteManifest` prueft deshalb vorab auf `TenantID == nil` und antwortet `ErrManifestImmutable` (403) — dieselbe Begruendung wie `preset_immutable` bei den Rollen.
- **`wasm_binary_hash` ist NULLable ohne Default**, das Model haelt es als `string`. Eine per Migration ausgelieferte Katalog-Zeile ohne diese Spalte haette jeden Read fuer **alle** Tenants zum Scan-Fehler gemacht. Die drei Lesequeries nutzen jetzt `COALESCE(wasm_binary_hash, '')`.
