...
>
> **★ OFFEN / NÄCHSTE UNIT (mit Darien zu besprechen):** (a) **restliche X-4-Store-Splits** (automatisierung/berichte/mail/formulare-Prefs — dialerTenant ist die Referenz) · (b) **Bexio-Invoice-Pull-FE reviewen** (Subagent-Bau) · (c) **weitere Welle-2-Demo-Tiefe** — admin-Lücken (Integrations-Tab-Placeholder), **video-tote-Buttons** (CallHistory/Header ohne onClick), notifications · (d) **Welle 3 Onboarding/Info-Center** (§1.2) · (e) **Echt-Schaltungs-Verifikation der Luke-bereinigten Module** (hr/security/automation/produktion/formulare visuell gegen echtes BE). **Luke-gebunden bleibt:** security-DSGVO-Echt-Schaltung, mails-IMAP, admin-Backend (Invite/RBAC/License/S3).
