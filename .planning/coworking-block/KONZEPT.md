...
- **Szene (Darien-Präzisierung 2026-07-20):** 2D-Illustration (Tisch/Raum) mit **festen Sitzplätzen als Ankerpunkten** — joint jemand, setzt sich sein Avatar auf den nächsten freien Platz (Spring-Pop-in), beim Verlassen wird der Platz frei. Raum-Variante + Platzanzahl werden beim Raum-Erstellen im Modul eingestellt. **Animations-Scope Phase A bewusst minimal:** dauerhaft laufende Mini-Loops (Atmen/Blinzeln) + EIN Sprech-Indikator — mehr nicht; Unterschiedlichkeit kommt rein aus dem Baukasten (Kopf/Farbe/Accessoire). KEIN Pixel-Look, keine Minigames, kein Timer-Widget (optionales „Ziel nennen beim Betreten" als leichtes Commitment-Feature prüfen).
- **Herstellungs-Entscheid (2026-07-20): SVG-Baukasten + Code-Animation** (geschichtete React-SVG-Komponenten `<CosmiCharacter head/color/accessory/state>`, Motion-Tokens, nur transform/opacity — Ressourcen-Budget!). Claude produziert Design + Animation + Code selbst, Iteration im Screenshot-Loop mit Darien; Proof-Start = 1 Charakter (Mars-Kopf) sitzend/atmend/blinzelnd/tippend. **Rive bewusst NICHT für 1.0** (.riv entsteht nur im Rive-Editor = menschliches GUI-Tool mit Lernkurve; Claude kann Rive nur einbinden, nicht erzeugen) — als Upgrade-Pfad notiert, falls Charaktere später deutlich lebendiger werden sollen.
