...
3. **team** ← NÄCHSTES MODUL. Markt-getrieben: Stammdaten-Export + Lohn-Schnittstelle, DATEV-HR DE im MVP, AT/CH später. + team-Moduleinstellungen. → **kommunikation** → **work** → … (Reihenfolge `module-phase-plans.md`). **mails zurückgestellt** (Backend-Arch mit Luke). **security ans Ende** (DSGVO mit Luke).
