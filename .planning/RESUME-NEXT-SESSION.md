...
**▶ NÄCHSTE SESSION: Phase 5 (Synergie + Moduleinstellungen-Rest + Review).** Feinplan-Abschnitt „Phase 5" in `batch-kommunikation-feinplan.md`. Inhalt: interne Notizen+@Kollegen im Kunden-Thread (InternalNoteComposer real — **Teil schon da:** ConversationThread `handleInternalNote` hängt jetzt interne Notiz an den Thread-Overlay an, direction `internal`), Collision-Hinweis (Mock-Presence), Call-Bridge ins video-Modul, Slash-Command-Mock-Shell, KommunikationSettingsPanel restliche tenant-Sektionen (Kanäle-Connect-Shell/Canned-Responses/Retention-Hinweis/Webhook-Config). Wie Phase 4: ZUERST echten Stand verifizieren.
