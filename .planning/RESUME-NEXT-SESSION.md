...
**✅ Backend-Handover für Luke ERSTELLT (2026-06-08): `.planning/backend-handover-luke.md`.** `backend-gaps.md` (organisch) → ein nach **Launch-Impact** priorisierter Lese-Plan: **P0 Launch-Blocker** (finanzen E-Rechnung/GoBD/DATEV/Bexio, kalender-Terminbuchung ZFA-Pilot, dialer-Consent-Risiko, Passwort-vergessen) · **P1 Feature-Parität** (kommunikation/work/kontakte/team scharfschalten + Settings-Fundament + Cross-cutting Quick Wins) · **P2 Post-Launch** (Branchen-Module Welle 3, admin/billing, automatisierung). Pro Punkt: was fehlt · Vorschlag (RPC/Endpoint/Tabelle) · FE-Status (🟢 wiring-ready / 🟡 teilweise / 🔴 FE wartet) + empfohlene Reihenfolge. `backend-gaps.md` bleibt als Detail-Arbeitsnotiz. **→ Darien gibt das Dokument Luke direkt.**
...
- ✅ **P5 = Kalender-Sicht — KOMPLETT (2026-06-08).** Eigenes leichtes Monatsgrid (`modules/work/calendar/calendar-utils.ts` Mo-ausgerichtet, ganze Wochen + lokale Date-Keys, timezone-sicher) + `WorkCalendarView.tsx`. Eingehängt als **5. View** im Projekt-Board-Toggle (`ProjectDetailPage`, Icon `CalendarDays` zwischen Gantt/Auslastung; ViewType-Union + pref-Sync + Persist um `'calendar'` erweitert). Tasks aus `useTasks({project_id})` nach `due_date` gebucketet; Prioritäts-Punkt-Chips, Überfällig-Markierung, Heute-Highlight, Wochenend-/Fremdmonats-Dimming, **„Ohne Fälligkeit"-Tray**. **Drag = Fälligkeit ändern** via dnd-kit (useDraggable-Chip / useDroppable-Tageszelle) → `useUpdateTask({due_date})` mit optimistischem Cache-Update. i18n ×4 (6 Keys: `work.views.calendar` + `work.calendar.*`). QA `scripts/qa-work-calendar.mjs` grün (Grid/Nav/Tray, 0 Raw-Keys/pageErrors/failedApi) + **Drag end-to-end verifiziert** (Chip wandert real in Zielzelle) + 2 Screenshots angesehen. tsc `tsconfig.wp5check.json`: eigene Calendar-Dateien clean, Rest = vorbest. Baseline-Rauschen (useTasks/useProjects generated-types, ActivityLog/CommentThread). backend-gaps: P5 = kein neuer BE-Bedarf (nutzt `PUT /tasks/{id}`). **→ work-Batch KOMPLETT (P1–P5).**
