...
- **▶ NÄCHSTES: P3 = Labels/Tags auf Tasks.** Label-Taxonomie existiert schon (`stores/workSettings.labels`). Jetzt: Chip-UI in Kanban-Card/List-Row/Task-Detail + Add/Remove-Picker (Taxonomie aus P2) + Filter in `TaskFilterBar`. Mock-first Overlay (label_ids an Task fehlt im Backend → backend-gaps notiert). Dann P4 (Portfolios + Auslastung/Budget echt) · P5 (Kalender-Sicht). Empfehlung: weiter Sonnet (Execute).

<details><summary>Alter P1-Hinweis (erledigt)</summary>

Direkt starten mit **P1 = Demo-Mode reparieren**: fehlende MSW-Handler in `desktop/src/renderer/src/mocks/handlers/work.ts` ergänzen. Exakte fehlende Pfade (Client nutzt sie schon, MSW fehlt → Demo bricht):
