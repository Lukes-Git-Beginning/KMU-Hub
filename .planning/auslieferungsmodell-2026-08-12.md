...
> Darien ↔ Claude, in der die offenen Punkte gegen den Code geprüft wurden. Alles unten mit
> Pfad:Zeile ist nachgeschaut, nicht geschätzt. Kein Produktionscode angefasst.
>
> **Zweck:** Diskussionsgrundlage für die Runde. Enthält Prämissen, die Darien gesetzt hat,
> und Befunde, die Lukes Etappenplan an mehreren Stellen verschieben.

---

## 1 · Was Darien gesetzt hat

Diese Punkte sind in der Runde entschieden worden und Voraussetzung für alles weitere:

1. **Kein Multi-Tenant-SaaS für Piloten.** Jeder Kunde bekommt einen **eigenen Hetzner-Server**.
   Begründung: geschlossenes System beim Kunden, Zentria greift nur mit erteilter Berechtigung zu.
2. **Der erste Pilot ist unentgeltlich**, Zugang für Zentria wird vorab abgesprochen.
3. **ORBIT ist vorerst raus** — Hardware, kommt später.
4. **Umfangreiche, unfertige Module fliegen aus dem 1.0** und werden nach dem Piloten fertig
   gebaut. Welche genau: **offen, wird in der Runde entschieden.**
5. **Editor-Rollout startet nicht mit `finanzen`**, sondern mit sicheren Modulen, die im 1.0
   bleiben — Kalender, Dokumente, evtl. Schichtplanung. Deckt sich mit Lukes Empfehlung.
6. **Website und Preisgestaltung** werden in den nächsten 1–2 Tagen separat überarbeitet.
7. **Nur gebuchte Module gehören auf den Kundenserver** — nicht ausliefern und abschalten.
   Wörtlich: die BMW-Sitzheizung ist kein Vorbild.
...
| Editor als eigenes OS-Fenster (`src/main/ipc/editor-window.ts`) | **erledigt** — Tab auf zweitem Bildschirm ist laut Darien in Ordnung |
| Screen-Share-Quellenauswahl (`features/video/ScreenSourcePicker.tsx`) | **irrelevant** — Meeting-Modul ist nicht im 1.0 |
| Desktop-Benachrichtigungen | gehen im Browser, mit Erlaubnisdialog |

Damit schrumpft der Umbau von 15 betroffenen Renderer-Dateien auf im Wesentlichen **eine**
(Token-Speicherung) plus die Fensterleiste.
...
## 3 · Was „eigener Server pro Kunde" am Code ändert
...
**Das ist unabhängig von Entscheidung 2 nötig** und steht in keiner bisherigen Planung.
...
Server**. Für klassisches SaaS wäre das zu grob — **bei einem Server pro Kunde ist es genau die
richtige Ebene.** Was für SaaS noch zu bauen wäre, existiert für dieses Modell bereits.

### 3.4 Modul weglassen statt abschalten — geht, und zwar richtig

Dariens Einwand ist technisch berechtigt: Auf einer Maschine, auf der der Kunde Root hat, ist ein
Flag keine Zugangskontrolle, sondern eine Bitte. **Die Architektur trägt echtes Weglassen bereits:**
...
**Formulierbare Zusage:** *„Module, die Sie nicht gebucht haben, sind auf Ihrem Server nicht
installiert."* Wörtlich wahr und prüfbar — passt zur Datensouveränitäts-Erzählung.
...
## 4 · Was das an Lukes Etappenplan verschiebt

### Fällt weg

| Posten | Grund |
|---|---|
| G0-11 Installer nie gebaut | entfällt bei Web |
| G0-12 Code-Signatur | entfällt bei Web (spart 150–750 €/Jahr) |
| Kein Update-Kanal (G2, 1,5 PT) | Server aktualisieren reicht |
| Screen-Share-Umbau | Meeting-Modul nicht im 1.0 |
| Editor-Fenster-Problem | Tab auf zweitem Bildschirm |

### Kommt neu dazu — steht in keiner Analyse

Lukes Streichliste sagt: *„ORBIT/Self-Hosted nicht kundenfähig machen — 4 PT, verkauft sich erst,
wenn jemand danach fragt."* **Dieser Posten ist bei „eigener Server pro Kunde" kein Streichkandidat,
sondern Voraussetzung.** Technisch ist es dasselbe Problem wie ORBIT — mehrere Deployments, die
nicht der Hauptserver sind. Der Unterschied ist nur, wem die Maschine gehört.
...
Diese G1-Posten skalieren jetzt mit der Kundenzahl:

- **`restore.sh` ist strukturell kaputt und nie ausgeführt** — jetzt pro Kundenmaschine relevant
...
wird durch mehrere Maschinen schärfer. Bei einem unentgeltlichen Piloten tragbar; ab dem zweiten
Kunden nicht mehr.

---

## 5 · Blocker-Stand nach dieser Runde
...
**Drei Blocker lösen sich von selbst auf** (G0-7, G0-8, G0-9), wenn Helpdesk, Finanzen oder
Fuhrpark aus dem 1.0 fliegen. Deshalb sollte die Modul-Streichliste vor allem anderen stehen —
sie entscheidet, welche Blocker überhaupt noch welche sind.

### Grobe Rechnung bis „zeigbar"

| | PT |
|---|---|
| G0 unverändert | ~7,9 |
| minus wegfallende Modul-Blocker (je nach Streichliste) | −0,25 bis −1,75 |
| plus Auslieferungsweg (neu) | +3 bis 5 |
| **Summe** | **~10–13** |

Lukes Etappen 0–2 lagen bei 8–12 PT gegenüber 12–16 verfügbaren Tagen. **Weiter machbar, aber der
Puffer ist weg** — der Auslieferungsweg kostet ungefähr das, was Installer und Signatur einsparen.

Nicht enthalten: G1 (~10 PT, vor dem ersten echten personenbezogenen Datensatz). Beim
unentgeltlichen Piloten mit echten Kundendaten wird davon mindestens Backup/Restore scharf — das
ist dann keine Kür mehr.

---

## 6 · Zu entscheiden in der Runde

1. **Modul-Streichliste für 1.0.** Was bleibt, was kommt nach dem Piloten. Bestimmt drei G0-Blocker
   und den Zuschnitt jeder Kundeninstallation. **Zuerst.**
2. **Web oder Desktop** (Lukes Entscheidung 2). Neue Argumente aus dieser Runde: der Editor ist
   kein Blocker; Screen-Share entfällt; der Anti-BMW-Anspruch ist im Web sauberer umsetzbar; die
   fest einkompilierte Serveradresse macht den Installer-Weg bei mehreren Kundenservern teuer.
3. **Registry ja/nein** — bzw. wie Kundenserver an Images kommen, ohne Quellcode zu bekommen.
4. **Serveradresse zur Laufzeit** — nötig in jedem Fall, unabhängig von Punkt 2.
5. **Website-Fassung und Preise** — läuft ohnehin separat, hängt aber an Punkt 1 (was verkauft ihr,
   und welche Module gibt es zum Start wirklich).
6. **Support-Zusage und Zugriffsweg** für den Piloten — wer außer Luke kommt an SSH-Key und
   Vault-Passwort.
