...
## 🏃 MARATHON-MODUS (ab 2026-06-09) — 3 Ströme parallel
> Lane-Zuteilung + Branch-/Kollisions-Regeln: `collision-map.md`. Tagesablauf: `multi-stream-workflow.md`. Starttexte: `HANDOFF-TEXTS.md`. Review pro Modul: `reviews/<modul>.md`.
> **Lanes:** Strom **N** (Nico) = wiki·formulare·berichte·notifications · Strom **D** (Dein-PC) = calendar·dokumente·zeiterfassung · Strom **L** (Luke nachm.) = vertraege·dashboard·profil (vorm. Backend-P0).
> Module unten sind mit „→ Strom X" markiert. Jeder Bot baut NUR seine Lane, auf `marathon/<strom>`, Reviews danach durch Darien.
...
### Settings-Fundament (VOR Phase 7, von Darien „zuerst bauen" gewählt)   — *(nicht im Marathon morgen)*
...
### Phase 7 — Kontakte-Einstellungen (auf dem Fundament)   — *(nicht im Marathon morgen)*
...
### Phase 8 — Finanzberatungs-Tiefe (Domänen-Input nötig)   — *(nicht im Marathon morgen)*
...
### Settings-Fundament   — *(nicht im Marathon morgen)*
...
### Phase 8 Finanzberatung   — *(nicht im Marathon morgen)*
...
### Cross-cutting (für den großen Durchlauf)   — *(nicht im Marathon morgen)*
...
### dialer  (FE Phase-1-komplett; LiveKit P0)   — *(nicht im Marathon morgen)*
...
### kommunikation/chat  (3-Panel-Chat da)   — *(nicht im Marathon morgen)*
...
### mails  (Mock-Handler da, **kein FE-Modul** — Neubau)   — *(nicht im Marathon morgen)*
...
### video/meetings  (volles Mock-UI, LiveKit P0)   — *(nicht im Marathon morgen)*
...
### calendar  (nur Layout-Shell, Views = Platzhalter)   — **→ Strom D**
...
### notifications  (Mock-Handler komplett, nur Widget-FE)   — **→ Strom N**
...
### work  (sehr vollständig: Projekte/Kanban/Gantt/Tasks/Timer)   — *(nicht im Marathon morgen)*
...
### zeiterfassung  (dünner Wrapper um Profil-Tab)   — **→ Strom D**
...
### dokumente  (sehr vollständig: Spaces/Versionen/OnlyOffice/KI)   — **→ Strom D**
...
### wiki  (volle UI auf Zustand-Store)   — **→ Strom N**
...
### team  (12 Tabs, Mitglieder/Leave via Query, Rest Mock)   — *(nicht im Marathon morgen)*
...
### helpdesk  (gut, auf Zustand-Store)   — *(nicht im Marathon morgen)*
...
### formulare  (sehr vollständig: Builder/Logik/Submissions)   — **→ Strom N**
...
### berichte  (4 Tabs, Hooks da, kaum echte Charts)   — **→ Strom N**
...
### finanzen  (13 Tabs, Kern backend-connected; Ausgaben/Transaktionen noch Zustand)   — *(nicht im Marathon morgen)*
...
### buchhaltung  (dead code, Migrations-Aufgabe)   — *(nicht im Marathon morgen)*
...
### vertraege  (vollständige FE-Only auf Zustand-Store)   — **→ Strom L**
...
### settings  (14 Tabs, ausgebaut) — **enthält das Settings-Fundament (VOR Kontakte-Phase 7)**   — *(nicht im Marathon morgen)*
...
### admin  (4 Tabs, RBAC-Guard)   — *(nicht im Marathon morgen)*
...
### security  (9 Sub-Seiten, viel Mock) — **DSGVO-Tools = P0-Launch-Blocker (R2)**   — *(nicht im Marathon morgen)*
...
### automatisierung  (sehr vollständig: Wizard/ReactFlow/Mock)   — *(nicht im Marathon morgen)*
...
### dashboard  (vollständig, Layout-Persistenz Mock)   — **→ Strom L**
...
### rapporte  (reif; Solar-Pilot-Kern) ⚠   — *(nicht im Marathon morgen)*
...
### schichten  (reif; ArbZG-Check da; Mitarbeiter hardcoded) ⚠   — *(nicht im Marathon morgen)*
...
### fuhrpark  (reif; GPS/Fahrtenbuch Mock) ⚠   — *(nicht im Marathon morgen)*
...
### vermietung  (vollständig) ⚠   — *(nicht im Marathon morgen)*
...
### inventar  (vollständig; Scan-Icon nur Mock) ⚠   — *(nicht im Marathon morgen)*
...
### einkauf  (vollständig)   — *(nicht im Marathon morgen)*
...
### produktion  (vollständig; MRP fehlt) ⚠   — *(nicht im Marathon morgen)*
