...
### Gesamtstand (verifiziert 23.06., aktualisiert 25.06. + 28.06. + 14.07.)
- **14.07.-Update (Luke-Wellen in Dariens Pause, main `adfa75ff`):** Luke hat drei Kampagnen durchgezogen (23 von 25 Commits seit 05.07.): (1) **Bexio Invoice-Pull Welle 3b** — Read-only-Mirror externer Rechnungen (Migr. 000243), FE via Subagent (Read-only-Badge/Banner, mutierende Aktionen ausgeblendet) → **Darien-Review offen**. (2) **protojson-Kampagne** (~13 Commits) — Proto-Timestamps modulweit von `{seconds,nanos}` auf ISO umgestellt (fixt FE-Datumsparsing gegen echtes BE). (3) **FE/BE-Contract-Mismatch-Kampagne** (6 Baustellen) — hr/security/automation/produktion/formulare/integration-Clients auf echte Wire-Shape ausgerichtet. **→ Welle-1-Echt-Schaltung modulweit de-riskt**; ⚠ visuelle Verifikation (Screenshot-QA gegen echtes BE) steht aus, weil Luke kein headless-Electron fahren konnte. Details: `.knowledge/troubleshooting.md` §FE/BE-Contract-Mismatch + `RESUME-NEXT.md` #7.
...
- [ ] **X-4** **FE-Settings localStorage → Backend** (ModuleSettingsShell-Persistenz, alle Module; BE Migr. 138 da) — **Referenz-Muster etabliert + live verifiziert 24.06. (dokumente):** store-direct `initFromServer()` (GET `/settings/{id}/tenant`, Hydrate on Panel-Mount) + setter-Write-Through (PUT) im Zustand-Store, gespiegelt von `stores/settings.ts`; localStorage bleibt optimistischer Cache. End-to-end verifiziert (localStorage gewiped → Panel hydratet documents+images/14d aus dem Backend; Screenshots `desktop/.qa-screenshots/settings-x4/`). **Rollout pro Modul-Store:** dashboard/work/wiki/helpdesk/automatisierung/berichte/formulare/vertraege/mails/finanzen/dialer/kommunikation/payroll/segment — je `initFromServer`+Write-Through nach dem dokumente-Muster (user-scope via `useUserSettings`, tenant-scope store-direct).
...
O-0 Recherche zuerst → O-1…O-6 (zusammenhängender Block). **Darien 2026-07-16: erst NACH dem Review des RBAC-Blocks starten** (Onboarding soll rollenbewusst auf dem finalen Stand aufsetzen).
