...
- [ ] **work + finanzen FE-Echt-Schaltung — verbleibende Wire-Shape-Fixes (B-12):** work `{seconds,nanos}`-Timestamps→ISO (Board/Gantt hängen sonst); finanzen protojson Enum-Int-Status→String + `useInvoice/useQuote` flat-select. Endpoints alle 200, Daten da. Details: `sub-terminal-work-finanzen-paket.md` (UPDATE-Block oben).
