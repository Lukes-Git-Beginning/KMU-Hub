...
- ✅ **CreateEmployee-Endpoint — TEILSCHNITT ERLEDIGT 2026-06-11 (`a3ad7158`):** `POST /api/v1/hr/employees` (hr:admin) legt Profil für EXISTIERENDEN User an (`user_id` im Body, AlreadyExists bei Duplikat). ⚠ Follow-up: Auth-User-Anlage (Invite-Flow: email + temporary_password + roles, transaktional) — FE-Wizard muss bis dahin auf User-Picker oder zweistufigen Flow (register → create profile) umgestellt werden.
...
- ✅ **Label-Taxonomie + Task-Labels — BACKEND ERLEDIGT 2026-06-11 (`2b8447b6`, Migrationen 000145+000147):** `work_labels`+`task_labels` (RLS), Label-CRUD `/api/v1/work/labels`, `PUT /tasks/{id}/labels`, `label_ids` im TaskProto, Permission-Seeds `work_labels:*`. Follow-ups: `label_ids` wird in Get/ListTasks noch nicht batch-geladen; `filter_label_ids` ist durchgereicht aber ohne SQL-JOIN im task-Repo; FE-Wiring (Chip-UI/Filter) = FE-Lane.
