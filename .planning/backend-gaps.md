...
> **⭐ STAND nach Lukes Backend-Welle (2026-07-30, Rebase auf origin/main):** Lukes Backend-Nachtloop hat **einen Teil der Ticket-Basis bereits geliefert** (schon 2026-06-28, Commit `d2473afb`, vor diesem Audit — Backlog-Units `p3-helpdesk-wire-ticket` + `p3-helpdesk-ticket-number` „done"): `Ticket` trägt jetzt `description`, `category`, `requester_name`, `assignee_name` (denormalisiert, tenant-gescopter LEFT JOIN) + fortlaufende `ticket_number` pro Tenant (Migration `000236_helpdesk_ticket_fields` mit `helpdesk_ticket_counters`, RLS aktiv, Bestand backfilled). Helpdesk-Writes sind seit der Welle tenant-scoped (`e0bccf02`). **→ Damit ist der erste 🔴-Punkt teilerfüllt** — beim FE→BE-Wiring die Feldnamen abgleichen (BE: `ticket_number` int / FE: Pseudo-`HD-YYYY-NNNN` aus UUID → BE-Nummer übernehmen). **Noch NICHT im BE (grep über Proto+Modell leer):** `channel`, `custom_fields`, `csat_*`, `requester_email`, `requester_is_external`, `intake_target`. Diese vier bleiben die echten Intake-Gaps.
> **⭐ Scope-Aufnahme bei Luke nötig:** Der Intake-Block ist **neuer** als Lukes Backlog — er steht dort nirgends (auch nicht im `blocked` Sammel-Unit `p3-fe-only-features-scope-decision`). Damit Luke bauen/streichen/vertagen entscheiden kann, muss Intake als eigener Bereich in dieses Backlog. Dieser Abschnitt hier ist die Quelle.

- 🔴 **`CreateTicketInput` erweitern** — teilerfüllt durch Lukes Welle (`description`, `category`, `requester_name` sind da). **Rest fehlt** (FE sendet bereits, MSW nimmt an): `channel` (`agent|selfservice|external`), `requester_email`, `requester_is_external`, `custom_fields` (Map). BE-Handler + Proto/pb + Service müssen nur diese vier nachziehen.
...
- 🟠 **Kanal-Konfiguration + Selfservice (P4/P6, FE gebaut Session #32, 2026-07-27):** `helpdesk`-Store trägt jetzt `intakeChannels {agent,selfservice,external}` + `intakeFormId` (tenant, funktionaler Toggle wie `csatEnabled` — NICHT über den Draft/Deploy-Overlay). Konfiguriert im Modul-Editor („Kanäle"-Panel, `KanaelePanel.tsx`), Sandbox reagiert live. **Kanal 2 (Intern-Selfservice) FE-fertig:** Einstieg in Settings→„Über Cosmi"→IT-Support, `IntakeFormFill` (shared, wiederverwendbar) rendert das gebundene Formular, requester = eingeloggtes Profil, channel=`selfservice`. **Herkunfts-Reiter** im Modul (Alle/Agent/Selfservice/Extern + „zusammenführen") filtert über `DisplayTicket.channel`. **BE-Bedarf:** `intake_channels`-Config + `intake_form_id` tenant-scoped persistieren; Cross-Window-Liveness (Editor schreibt in eigenem Fenster → Haupt-App liest bei Reload). **⚠ Cross-Window:** aktuell localStorage-persist, kein Live-Sync zwischen Editor-Fenster und Haupt-App (Reload holt es) — sauberer: BroadcastChannel wie die Customization-Sync.
- 🔴 **Öffentliche Formular-Route** (`/r/:token`, kein Login) — Haupt-Blocker für Kanal 1 (extern, P5). Extern-Toggle im Editor existiert (mit „Route noch nicht live"-Hinweis), aber echte Einreichung von außen braucht die Route + Requester-Objekt + Anti-Spam/Rate-Limit + echtes Datei-Storage. **P5 bewusst NICHT FE-fake gebaut** — wartet auf Luke.
  > **⭐ Muster existiert schon (Lukes Welle):** Luke hat für Berichte eine **unauthentifizierte Token-Route** gebaut — `POST /api/v1/public/berichte/reports/{token}` (Backlog `p3-berichte-share-token` done, Iteration 34, Migration `000252`). Sie löst genau die sicherheitskritischen Teile, die die Intake-Route auch braucht: **ausreichende Token-Entropie, konstantzeitiger Vergleich, hartes serverseitiges Ablauf-Enforcement, Rate-Limit, kein Enumerieren, nur genau die freigegebene Ressource (nie tenant-weit), abgelaufen/widerrufen → 404 statt 403.** → **P5 = dieses Muster auf ein `intake_form_submit`-Ziel anwenden**, statt es neu zu entwerfen. Das macht P5 für Luke deutlich billiger; der Intake-spezifische Rest ist nur Requester-Aufnahme (Name/E-Mail extern) + Datei-Upload + die Formular→Ticket-Dispatch-Logik (siehe P2-Punkt oben).
