...
### dokumente
- ⚠ **Presign-Upload braucht öffentlichen MinIO-Endpoint (2026-06-11, Backend-Session):** Die neuen Presign-Endpoints (`POST /api/v1/files/presign-upload`, Commit `13a7b90a`) signieren gegen `MINIO_ENDPOINT=minio:9000` (interner Docker-Hostname); Caddy hat keine MinIO-Route → die URLs sind aus Browser/Electron NICHT erreichbar. Vor FE-Nutzung nötig: (1) Caddy-Route (Empfehlung: Subdomain `s3.zentria.tech` → `reverse_proxy minio:9000`, plus DNS), (2) neue Env `MINIO_PUBLIC_ENDPOINT` + zweiter minio-go-Client im document-Service nur fürs Presigning (Signatur deckt den Host ab!), (3) MinIO-CORS (`api cors_allow_origin`) auf die App-Origin. Aufwand ~S. Bis dahin ist der Presign-Pfad nur server-intern nutzbar.
...
### team
- **⚠ FE↔BE-Shape-Mismatch HR-Employees (Befund 2026-06-11):** `EmployeeProfile` in `desktop/src/renderer/src/types/hr-types.ts` und alle team-Modul-Consumer (team-Modul, Mentions, Assignee-Listen, Kommunikation-Posteingang) erwarten camelCase (`userName`, `firstName`, `lastName` usw.). Das echte Gateway serialisiert `hr.pb.go`-Proto-Structs via `encoding/json` mit snake_case-Tags (`user_name`, `first_name`, `last_name`). Es gibt keinen Transform-Layer im hr-client. Demo-Mode ist seit 2026-06-11 (`7a367047`) camelCase-konsistent (Mock-Handler angeglichen) — gegen das echte Backend würde das team-Modul „Unbekannt" anzeigen. **Vor API-Swap des team-Moduls:** entweder Adapter/Transform im hr-API-Client (`api/clients/hrClient.ts`) ODER Gateway-DTO mit camelCase-JSON-Tags in `route_hr.go`. Kein Migration-Bedarf, rein serialisierungsbedingt.
