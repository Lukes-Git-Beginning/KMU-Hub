...
// QA: berichte R6-2 — vertraege (Verträge) + einkauf (Bestellungen) sources in the
// chart-block SourcePicker render a live preview.
// Run: node scripts/qa-berichte-r6-2.mjs   (dev server on :5173)
const BASE = 'http://localhost:5173'
const outDir = resolve('.qa-screenshots/berichte-r6-2')
...
try {
  ...
  out.sourcesVisible = await page.evaluate(() => {
    ...
    return { vertraege: t.includes('Verträge'), einkauf: t.includes('Bestellungen') }
  })
  await shot(page, '1-source-picker.png')

  out.vertraegeCharts = await pickSource(page, 'Verträge', /Vertragsart/, /Vertragswert/, '2-vertraege-preview.png')
  out.einkaufCharts = await pickSource(page, 'Bestellungen', /Kategorie/, /Bestellwert/, '3-einkauf-preview.png')
