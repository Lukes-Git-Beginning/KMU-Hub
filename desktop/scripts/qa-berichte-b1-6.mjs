...
// Verkaufsbericht Q2 (released -> read mode, has chart + KPI row + callout)
const card = page.getByRole('button').filter({ hasText: /Verkaufsbericht Q2/ }).first()
await card.click().catch(() => {})
await page.waitForTimeout(2800) // chart fetch + render
out.reader = {
  rawKeys: await page.evaluate(findRawKeys, RAW.source),
  errs: errs.length,
  charts: await page.locator('svg.recharts-surface').count(),
}
await page.screenshot({ path: resolve(outDir, 'b1-6-reader.png'), fullPage: true })
