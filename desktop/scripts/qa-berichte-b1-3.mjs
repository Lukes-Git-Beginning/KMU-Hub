...
// Open Monatsbericht (status final -> editable)
...
await page.screenshot({ path: resolve(outDir, 'b1-3-editor-edit.png') })

// Open the insert picker
const addBtn = page.getByRole('button', { name: /Block einfügen/ }).first()
if (await addBtn.count()) {
  await addBtn.click().catch(() => {})
  await page.waitForTimeout(400)
  await page.screenshot({ path: resolve(outDir, 'b1-3-insert-picker.png') })
}

// Toggle to read mode
const readBtn = page.getByRole('button', { name: /^Lesen$/ }).first()
if (await readBtn.count()) {
  await readBtn.click().catch(() => {})
  await page.waitForTimeout(400)
  out.read = { errs: errs.length }
  await page.screenshot({ path: resolve(outDir, 'b1-3-read-mode.png') })
}
