    ...
    <Suspense
      fallback={
        <div className="flex items-center justify-center h-64">
          <p className="text-sm text-muted-foreground">{t('security.admin.loading')}</p>
        </div>
      }
    >
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="w-56 shrink-0 border-r border-border bg-card p-4 overflow-y-auto">
        <div className="flex items-center gap-2 px-2 mb-4">
          ...
          <h3 className="text-sm font-medium text-foreground">{t('security.admin.title')}</h3>
        </div>
        <nav className="space-y-4">
          {TAB_GROUPS.map((group) => {
            const groupTabs = ALL_TABS.filter((tab) => tab.group === group)
            return (
              <div key={group}>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground px-3 mb-1">
                  {t(`security.admin.group.${group.toLowerCase()}`)}
                </p>
