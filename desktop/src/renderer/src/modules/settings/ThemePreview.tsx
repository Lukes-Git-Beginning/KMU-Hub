...
const PALETTE_LABEL_KEYS: Record<ColorTheme, string> = {
  graphit: 'settings.theme.palette.graphite',
  sand: 'settings.theme.palette.sand',
  ozean: 'settings.theme.palette.ocean',
}
...
export function ThemePreview({ className = '' }: { className?: string }) {
  ...
  const layoutLabel =
    ...
    navLayout === 'dock' ? 'Dock' :
    navLayout === 'topnav' ? 'Top-Nav' : t('settings.theme.layout.compact')
  ...
  return (
    <div className={`rounded-xl border border-border overflow-hidden ${className}`}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-secondary/50 border-b border-border">
        <span className="text-xs font-medium text-foreground">{t('settings.theme.livePreview')}</span>
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">
            {t(PALETTE_LABEL_KEYS[colorTheme])}
          </span>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex-1 p-2.5 overflow-hidden">
            ...
            <div className="flex gap-2 mb-2.5">
              {[
                { label: t('settings.theme.preview.revenue'), value: '42.8k', color: 'var(--primary)' },
                { label: t('settings.theme.preview.customers'), value: '184', color: 'var(--accent-1)' },
                { label: t('settings.theme.preview.projects'), value: '12', color: 'var(--accent-2)' },
              ].map((stat) => (
                <div
                  key={stat.label}
            ...
            >
              ...
              {[0, 1, 2].map((i) => (
                ...
                >
                  ...
                  >
                    {i === 0 ? t('settings.theme.preview.active') : i === 1 ? t('settings.theme.preview.open') : t('settings.theme.preview.new')}
                  </div>
                </div>
              ))}
            </div>
