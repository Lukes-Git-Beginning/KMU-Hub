...
func TestTokenMaker_CreateAndValidateAccessToken(t *testing.T) {
	...
	token, err := tm.CreateAccessToken(userID, tenantID.String(), roles, &TokenPermissions{Permissions: perms})
...
func TestTokenMaker_TenantID_Roundtrip(t *testing.T) {
	...
	token, err := tm.CreateAccessToken(userID, tenantID.String(), nil, &TokenPermissions{})
	require.NoError(t, err)
...
func TestTokenMaker_TenantID_EmptyLegacy(t *testing.T) {
	...
	token, err := tm.CreateAccessToken(userID, "", nil, &TokenPermissions{})
	require.NoError(t, err)
...
func TestTokenMaker_ExpiredToken(t *testing.T) {
	...
	token, err := tm.CreateAccessToken(userID, uuid.New().String(), nil, &TokenPermissions{})
	require.NoError(t, err)
...
func TestTokenMaker_InvalidSecret(t *testing.T) {
	...
	token, err := tm1.CreateAccessToken(uuid.New(), uuid.New().String(), nil, &TokenPermissions{})
	require.NoError(t, err)
...
func TestTokenMaker_TamperedToken(t *testing.T) {
	...
	token, err := tm.CreateAccessToken(uuid.New(), uuid.New().String(), nil, &TokenPermissions{})
	require.NoError(t, err)
