...
func (r *PostgresIntegrationConfigRepo) GetByPlatform(ctx context.Context, platform string) (*IntegrationConfig, error) {
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, fmt.Errorf("lexware: integration config not found for platform %s", platform)
		}
		return nil, fmt.Errorf("lexware: get config by platform: %w", err)
	}
...
func (r *PostgresIntegrationConfigRepo) Upsert(ctx context.Context, config *IntegrationConfig) error {
	...
	if err != nil {
		return fmt.Errorf("lexware: marshal config metadata: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("lexware: upsert config: %w", err)
	}
