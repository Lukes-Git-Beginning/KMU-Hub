...
func ParseCII(xmlData []byte) (*ParsedInvoice, error) {
	...
	if err := assertInboundTotalsConsistent(lineItems, subtotal, totalTax, grossTotal); err != nil {
		return nil, err
	}
...
// assertInboundTotalsConsistent rejects a foreign e-invoice whose own declared
// totals do not add up: the sum of its line totals must match its declared
// subtotal, and subtotal + tax must match the declared gross total. A document
// that fails this is either corrupt or was tampered with in transit, and either
// way must not be silently booked — the caller (Service.Import) has no other
// point where these three header figures and the line items are all in scope
// together.
//
// Tolerance reuses totalsTolerance (generator_doc.go): the same half-a-cent-per-
// line rounding slack this product already accepts on the outbound side, so an
// inbound document generated by this product's own outbound path always
// roundtrips.
func assertInboundTotalsConsistent(lineItems []ParsedLineItem, subtotal, totalTax, grossTotal decimal.Decimal) error {
	tolerance := totalsTolerance(len(lineItems))

	lineSum := decimal.Zero
	for _, li := range lineItems {
		lineSum = lineSum.Add(li.LineTotal)
	}
	if lineSum.Sub(subtotal).Abs().GreaterThan(tolerance) {
		return fmt.Errorf("%w: line items sum to %s but declared subtotal is %s", ErrParseFailed, lineSum.StringFixed(2), subtotal.StringFixed(2))
	}

	if subtotal.Add(totalTax).Sub(grossTotal).Abs().GreaterThan(tolerance) {
		return fmt.Errorf("%w: subtotal %s plus tax %s does not match declared gross total %s", ErrParseFailed, subtotal.StringFixed(2), totalTax.StringFixed(2), grossTotal.StringFixed(2))
	}
