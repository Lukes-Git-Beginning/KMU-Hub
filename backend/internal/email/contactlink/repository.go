...
func (r *PostgresRepository) Create(ctx context.Context, link *models.EmailContactLink) error {
	...
	_, err := r.pool.Exec(ctx,
		`INSERT INTO email_contact_links (id, message_id, contact_id, link_type, created_at)
		 VALUES ($1, $2, $3, $4, $5)
		 ON CONFLICT (message_id, contact_id) DO NOTHING`,
		link.ID, link.MessageID, link.ContactID, link.LinkType, link.CreatedAt,
	)
	return err
}

// GetByMessageID returns all CRM contact links for an email message.
func (r *PostgresRepository) GetByMessageID(ctx context.Context, messageID uuid.UUID) ([]*models.EmailContactLink, error) {
	rows, err := r.pool.Query(ctx,
		`SELECT id, message_id, contact_id, link_type, created_at
		 FROM email_contact_links WHERE message_id = $1
	...
	for rows.Next() {
		...
		if err := rows.Scan(&link.ID, &link.MessageID, &link.ContactID, &link.LinkType, &link.CreatedAt); err != nil {
			return nil, err
		}
...
// GetByContactID returns all email messages linked to a CRM contact, with pagination.
func (r *PostgresRepository) GetByContactID(ctx context.Context, contactID uuid.UUID, page, perPage int) ([]*models.EmailMessage, int, error) {
	offset := (page - 1) * perPage

	// Count total
	var total int
	err := r.pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM email_contact_links WHERE contact_id = $1`, contactID,
	).Scan(&total)
	if err != nil {
		return nil, 0, err
	}
...
// Delete removes a link between a message and a contact.
func (r *PostgresRepository) Delete(ctx context.Context, messageID, contactID uuid.UUID) error {
	_, err := r.pool.Exec(ctx,
		`DELETE FROM email_contact_links WHERE message_id = $1 AND contact_id = $2`,
		messageID, contactID,
	)
	return err
}
