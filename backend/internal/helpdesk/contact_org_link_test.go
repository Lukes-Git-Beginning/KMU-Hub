...
func TestCreateTicket_RejectsForeignTenantContactAndOrg(t *testing.T) {
	...
	_, err := svc.CreateTicket(ctxOwn, tenantOwn, uuidPtr(uuid.New()), "Ticket with foreign contact", TicketPriorityNormal, nil, nil, "", "", &foreignContactID, nil, TicketIntake{})
	if !errors.Is(err, ErrContactNotFound) {
		t.Fatalf("CreateTicket with foreign contact_id: expected ErrContactNotFound, got %v", err)
	}

	_, err = svc.CreateTicket(ctxOwn, tenantOwn, uuidPtr(uuid.New()), "Ticket with foreign org", TicketPriorityNormal, nil, nil, "", "", nil, &foreignOrgID, TicketIntake{})
...
func TestCreateAndUpdateTicket_LinksOwnTenantContactAndOrgAndFilters(t *testing.T) {
	...
	linked, err := svc.CreateTicket(ctxOwn, tenantOwn, uuidPtr(uuid.New()), "Linked ticket", TicketPriorityNormal, nil, nil, "", "", &contactID, &orgID, TicketIntake{})
	if err != nil {
	...
	unlinked, err := svc.CreateTicket(ctxOwn, tenantOwn, uuidPtr(uuid.New()), "Unlinked ticket", TicketPriorityNormal, nil, nil, "", "", nil, nil, TicketIntake{})
	if err != nil {
...
func TestUpdateTicket_RejectsForeignTenantContact(t *testing.T) {
	...
	ticket, err := svc.CreateTicket(ctxOwn, tenantOwn, uuidPtr(uuid.New()), "Update target", TicketPriorityNormal, nil, nil, "", "", nil, nil, TicketIntake{})
	if err != nil {
