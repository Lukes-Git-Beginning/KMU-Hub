...
func (r *PostgresRepository) Create(ctx context.Context, tag *models.Tag) error {
	_, err := r.pool.Exec(ctx,
		`INSERT INTO tags (id, name, color, entity_type, created_at)
		 VALUES ($1, $2, $3, $4, $5)`,
		tag.ID, tag.Name, tag.Color, tag.EntityType, tag.CreatedAt,
	)
	return err
}

func (r *PostgresRepository) GetByID(ctx context.Context, id uuid.UUID) (*models.Tag, error) {
...
func (r *PostgresRepository) GetByEntityAndName(ctx context.Context, entityType models.EntityType, name string) (*models.Tag, error) {
	return r.scanTag(r.pool.QueryRow(ctx,
		`SELECT id, name, color, entity_type, created_at
		 FROM tags WHERE entity_type = $1 AND LOWER(name) = LOWER($2)`, entityType, name,
	))
}

func (r *PostgresRepository) List(ctx context.Context, entityType *models.EntityType, offset, limit int) ([]*models.Tag, int, error) {
	var total int
	var countErr error

	if entityType != nil {
		countErr = r.pool.QueryRow(ctx,
			`SELECT COUNT(*) FROM tags WHERE entity_type = $1`, *entityType,
		).Scan(&total)
	} else {
		countErr = r.pool.QueryRow(ctx,
...
func (r *PostgresRepository) IsInUse(ctx context.Context, id uuid.UUID) (bool, error) {
	...
	err := r.pool.QueryRow(ctx, `
		SELECT (
			...
			SELECT COUNT(*) FROM company_tags WHERE tag_id = $1
		) + (
			SELECT COUNT(*) FROM deal_tags WHERE tag_id = $1
		) + (
