...
func main() {
	...
	repo := rapporte.NewPostgresRepository(pool)
	rapporteService := rapporte.NewService(repo)
	...
	lis, err := net.Listen("tcp", cfg.RapporteGRPCPort)
	if err != nil {
		slog.Error("failed to listen", "port", cfg.RapporteGRPCPort, "error", err)
		os.Exit(1)
	}
	...
	go func() {
		slog.Info("health/metrics server starting", "port", cfg.RapporteHealthPort)
	...
	go func() {
		slog.Info("rapporte service starting", "port", cfg.RapporteGRPCPort)
	...
	select {
	case <-quit:
		slog.Info("shutting down rapporte service")
	case <-ctx.Done():
	}
	...
	slog.Info("rapporte service stopped")
}
